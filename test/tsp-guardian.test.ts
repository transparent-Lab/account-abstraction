...
describe('Guardian', function () {
  ...
  before(async function () {
    ...
    const act = await createTSPAccount(ethers.provider.getSigner(), accountOwner.address, entryPoint, guardian, 'code')
  ...
  it('any address should be able to call register', async () => {
    ...
    const { proxy: account } = await createTSPAccount(ethers.provider.getSigner(), accounts[1], entryPoint, guardian, 'code')
